...
class CallejoneadasManager {
  ...
  async cargarPersonalDisponible() {
    try {
      ...
      const container = document.getElementById(
        "personal_checkboxes_container",
      );
      if (container) {
        container.innerHTML =
          '<p class="loading-text" style="color: #e74c3c;">Error al cargar personal. Reintente.</p>';
      }
    }
  }
  ...
  actualizarCheckboxesPersonal() {
    ...
    if (this.personalDisponible.length === 0) {
      container.innerHTML =
        '<p class="loading-text">No hay personal disponible en la base de datos.</p>';
      return;
    }
  ...
  handleCheckboxChange(checkbox) {
    ...
    const selected = document.querySelectorAll(
      'input[name="personal_ids"]:checked',
    );
  ...
  async cargar() {
    try {
      ...
      this.mostrarStatus(
        "Error cargando callejoneadas: " + error.message,
        "error",
      );
    }
  }
  ...
  async aplicarFiltros() {
    ...
    try {
      ...
      this.mostrarStatus(
        `Filtro aplicado: ${this.data.length} resultado(s)`,
        "success",
      );
    } catch (error) {
  ...
  render() {
    ...
    this.data.forEach((callejoneada) => {
      const estatusClass =
        callejoneada.estatus === "Pagada" ? "estatus-pagada" : "estatus-deuda";
      const participantes =
        callejoneada.personal_nombres &&
        callejoneada.personal_nombres.length > 0
  ...
  async editar(id) {
    try {
      ...
      this.mostrarStatus(
        "Error cargando callejoneada: " + error.message,
        "error",
      );
    }
  }
  ...
  async guardar(event) {
    ...
    const personalIds = Array.from(
      document.querySelectorAll('input[name="personal_ids"]:checked'),
    )
  ...
  async eliminar(id) {
    if (!confirm("¿Estás seguro de que quieres eliminar esta callejoneada?"))
      return;
