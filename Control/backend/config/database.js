...
// ── Detección de rutas en modo .exe (pkg) ──
// Buscamos la BD en este orden de prioridad:
//   1. Junto al .exe  →  <exeDir>/data/app.db          (ej. Control/data/)
//   2. Un nivel arriba → <exeDir>/../data/app.db        (ej. cuando el .exe está en dist/)
//   3. Roaming AppData → %APPDATA%\MiApp\database\app.db (instalación fresca)
...
if (isPkg) {
  ...
  const candidate1 = path.join(exeDir, "data");
  const candidate2 = path.resolve(path.join(exeDir, "..", "data"));
  const roaming = path.join(
    process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"),
    "MiApp",
    "database",
  );

  if (fs.existsSync(path.join(candidate1, "app.db"))) {
    // Caso ideal: data/ está al lado del .exe
    dataDir = candidate1;
    uploadsDir = path.join(exeDir, "uploads");
    console.log("📂 BD local detectada (junto al .exe).");
  } else if (fs.existsSync(path.join(candidate2, "app.db"))) {
    // .exe está en una subcarpeta (ej. dist/), data/ está un nivel arriba
    dataDir = candidate2;
    uploadsDir = path.resolve(path.join(exeDir, "..", "uploads"));
    console.log("📂 BD detectada en directorio padre (estructura dist/).");
  } else {
    // Sin BD local → usar Roaming para persistencia entre versiones
    dataDir = roaming;
    uploadsDir = path.join(roaming, "..", "uploads");
    console.log("📂 BD en AppData Roaming (instalación nueva).");
  }
} else {
  // Modo desarrollo normal (node server.js)
...
function initDatabase() {
  db.serialize(() => {
    ...
    db.run(
      `CREATE INDEX IF NOT EXISTS idx_callejoneadas_fecha ON callejoneadas(fecha_evento)`,
    );
    db.run(
      `CREATE INDEX IF NOT EXISTS idx_rel_calle           ON callejoneada_personal(callejoneada_id)`,
    );
    db.run(
      `CREATE INDEX IF NOT EXISTS idx_rel_personal        ON callejoneada_personal(personal_id)`,
    );
    db.run(
      `CREATE INDEX IF NOT EXISTS idx_documents_personal  ON documents(personal_id)`,
    );
...
function ensureColumns() {
  ...
  db.all(`PRAGMA table_info('callejoneadas')`, (err, rows) => {
    ...
    if (!cols.has("lugar_inicio"))
      add(`ALTER TABLE callejoneadas ADD COLUMN lugar_inicio TEXT DEFAULT ''`);
    if (!cols.has("lugar_fin"))
      add(`ALTER TABLE callejoneadas ADD COLUMN lugar_fin TEXT DEFAULT ''`);
    if (!cols.has("hora_inicio"))
      add(`ALTER TABLE callejoneadas ADD COLUMN hora_inicio TEXT DEFAULT ''`);
    if (!cols.has("hora_fin"))
      add(`ALTER TABLE callejoneadas ADD COLUMN hora_fin TEXT DEFAULT ''`);
  });
...
function migrateFromDocControl() {
  ...
  try {
    ...
    console.warn("⚠️ Error leyendo db.json:", e.message);
    return;
  }
      ...
      insertEmp.run(
        ...
        function (e) {
          if (!e) idMap[emp.id] = this.lastID;
            ...
            db.get(
              "SELECT id FROM personal WHERE email = ?",
              [email],
              (_e2, r) => {
                if (r) idMap[emp.id] = r.id;
              },
            );
          if (--pending === 0)
            insertEmp.finalize(() => insertDocs(documents, idMap));
        },
      );
    });
  });
...
function insertDocs(documents, idMap) {
  ...
  const stmt = db.prepare(
    `INSERT OR IGNORE INTO documents (personal_id,type,date,title,file_name,file_path,file_type,printed) VALUES (?,?,?,?,?,?,?,?)`,
  );
  documents.forEach((d) => {
    stmt.run([
      idMap[d.empId] || null,
  ...
  stmt.finalize(() =>
    console.log(`✅ Migración completada: ${documents.length} documento(s)`),
  );
}
...
function migrateUploads() {
  ...
  for (const oldDir of oldDirs) {
    ...
    try {
      ...
      if (migrated > 0)
        console.log(`📋 Migrados ${migrated} archivo(s) a ${uploadsDir}`);
    } catch (e) {
      /* silencioso */
    }
  }
